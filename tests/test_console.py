...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            cons.onecmd('create City name="Cairo"')
            ...
            clear_stream(cout)
            cons.onecmd('create User name="Sahar" age=16 height=5.6')
            mdl_id = cout.getvalue().strip()
            self.assertIn('User.{}'.format(mdl_id), storage.all().keys())
            clear_stream(cout)
            cons.onecmd('show User {}'.format(mdl_id))
            self.assertIn("'name': 'Sahar'", cout.getvalue().strip())
            self.assertIn("'age': 16", cout.getvalue().strip())
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User email="s.mfr@gmail.com" password="012"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
            ...
            self.assertIn('s.mfr@gmail.com', result)
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="s.mohamed@gmail.com", password="0123")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            result = cursor.fetchone()
            self.assertTrue(result is not None)
            self.assertIn('s.mohamed@gmail.com', result)
            self.assertIn('0123', result)
            self.assertIn('s.mohamed@gmail.com', cout.getvalue())
            self.assertIn('0123', cout.getvalue())
            cursor.close()
            dbc.close()
